...
class Users extends API_Controller
{
    ...
    function user_delete_post( ) {
        ...
        if ( $user_data->user_id == "" ) {
            ...
            $this->error_response( get_msg( 'user_not_able_delete' ), 403);
        } else {
    ...
    function apply_blue_mark_post()
    {
        ...
        if ($is_verify_blue_mark == 0 || $is_verify_blue_mark == 3) {
            ...
            if ($this->Blue_mark->save($user_data)) {
                ...
                $this->success_response( get_msg( 'apply_success' ), 201);
            } else {
                $this->error_response( get_msg( 'err_model' ), 500);
            }
        } else if($is_verify_blue_mark == 2) {
            $this->error_response( get_msg( 'blue_mark_pending' ), 400);
        } else {
            $this->error_response( get_msg( 'already_blue_mark' ), 400);
        }
