...
class Transactions extends BE_Controller {
	...
	function export_csv(){
		...
		// get user data 
		$data = $this->Paid_item->get_all()->result_array();
		
		if(count($data) != 0){
			// file creation 
			$file = fopen('php://output', 'w');

			$header = array(get_msg('no'), get_msg('item_name_label'), get_msg('amount_label'), get_msg('payment_method_label'), get_msg('status_label'), get_msg('start_date_label'), get_msg('end_date_label'));
			fputcsv($file, $header);
			$i=1;
			foreach($data as $key=>$value){
				...
				if ($today_date >= $value['start_date'] && $today_date <= $value['end_date']) {
					$status = get_msg('progress_label'); 
				} elseif ($today_date > $value['start_date'] && $today_date > $value['end_date']) {
					$status = get_msg('finished_label');
				} else {
					$status = get_msg('not_yet_start_label');
				}
				
				$line = array($i++, $this->Item->get_one($value['item_id'])->title, $value['amount'] . $this->Paid_config->get_one('pconfig1')->currency_symbol, $value['payment_method'], $status, $value['start_date'], $value['end_date']);
				fputcsv($file, $line);
			}
