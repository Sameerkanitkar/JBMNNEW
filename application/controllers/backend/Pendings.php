...
class Pendings extends BE_Controller {
	...
	function __construct() {
		...
		parent::__construct( MODULE_CONTROL, 'PENDINGS' );
	...
	function save( $id = false ) {
		...
		$id = ( !$id )? $data['id']: $id ;
		$title = $this->Pending->get_one($id)->title;
		//// Start - Send Noti /////
		if($data['status'] == 1) {
			...
			$updated_flag = $this->Pending->get_one($id)->updated_flag;
		...
		$user_ids = $this->Pending->get_one($id)->added_user_id;

		//for user name
		$user_id = $this->Pending->get_one($id)->added_user_id;
