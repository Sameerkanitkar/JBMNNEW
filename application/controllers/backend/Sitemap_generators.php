...
<sitemapindex xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
	<sitemap>
		<loc>' . $domain_sitemap . 'manufacturers.xml</loc>
		<lastmod>' . $today . '</lastmod>
	</sitemap>
	<sitemap>
		<loc>' . $domain_sitemap . 'models.xml</loc>
		<lastmod>' . $today . '</lastmod>
	</sitemap>
	<sitemap>
	...
	<sitemap>
		...
		// get manufacturer sitemap file
		$data = $this->Manufacturer->get_all();
		$domain_sitemap = $domain_name;
		$upload_path = "uploads/sitemaps/";
		$filename = $upload_path . "manufacturers.xml";
		$flag = "manufacturer";
		$this->create_sitemap($data, $domain_sitemap, $filename, $upload_path, $flag);

		// get model sitemap file
		$data = $this->Model->get_all();
		$domain_sitemap = $domain_name;
		$upload_path = "uploads/sitemaps/";
		$filename = $upload_path . "models.xml";
		$flag = "model";
		$this->create_sitemap($data, $domain_sitemap, $filename, $upload_path, $flag);
		
		// get product manufacturer sitemap file
		$manus = $this->Manufacturer->get_all();
		foreach($manus->result() as $manu){
			$domain_sitemap = $domain_name . 'item/';
			$upload_path = "uploads/sitemaps/products/";
			$filename = $upload_path . strtolower($manu->name) . ".xml";
			$conds['manufacturer_id'] = $manu->id;
...
<sitemapindex xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
		...
		if($flag == "product"){
			...
			$txt .= '<sitemap>
		<loc>' . $domain . 'sitemaps/products/'.strtolower($item->name).'.xml' . '</loc>
		<lastmod>' . $today . '</lastmod>
	</sitemap>
	';
			}
		...
		if($data){
			...
			foreach($data->result() as $item){
				if($flag == "manufacturer"){
					$txt .= "
	<url>
		<loc>". $domain . "item-list?manufacturer_id=".$item->manufacturer_id ."&amp;manufacturer_name=". str_replace(' ', '-', strtolower($item->name)) ."</loc>
		<lastmod>" . $today . "</lastmod>
		<changefreq>always</changefreq>
				...
				if($flag == "model"){
					...
					$txt .= "
	<url>
		<loc>". $domain . "item-list?manufacturer_id=".$item->manufacturer_id ."&amp;manufacturer_name=".
		str_replace(' ', '-', strtolower($this->Manufacturer->get_one($item->manufacturer_id)->name)).
		"&amp;model_id=".$item->id ."&amp;model_name=". str_replace(' ', '-', strtolower($item->name)) 
		."</loc>
		<lastmod>" . $today . "</lastmod>
		<changefreq>always</changefreq>
	</url>";
