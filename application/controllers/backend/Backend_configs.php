...
class Backend_configs extends BE_Controller {
	...
	function __construct() {
		...
		parent::__construct( MODULE_CONTROL, 'BACKEND CONFIGS' );
	...
	function save( $id = false ) {
		...
		// app_token
		if ( $this->has_data( 'app_token' )) {
			$data['app_token'] = $this->get_data( 'app_token' );
		}
		...
		//if 'email_verification_enabled' is checked
		...
		// topics_fe
		if ( $this->has_data( 'topics_fe' )) {
			$data['topics_fe'] = $this->get_data( 'topics_fe' );
		}
