...
class Ratings extends BE_Controller {
	...
	*/
	...
	function search() {
		...
		if($this->input->post('submit') != NULL ){
			...
			if($this->input->post('searchterm') != "") {
				$conds['searchterm_most_rating'] = $this->input->post('searchterm');
			...
			if($this->session->userdata('searchterm') != NULL){
				$conds['searchterm_most_rating'] = $this->session->userdata('searchterm');
	...
	function export_csv(){
		...
		$header = array(get_msg('user_name'), get_msg('overall_rating'), get_msg('added_date'), get_msg('verify_blue_mark'));
		fputcsv($file, $header);

		foreach($data as $key=>$value){
			$line = array($value['user_name'],  $value['overall_rating'], $value['added_date'], $value['is_verify_blue_mark']);
			fputcsv($file, $line);
		}
