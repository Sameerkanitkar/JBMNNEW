...
class Language_strings extends BE_Controller {
	...
	function __construct() {
		...
		parent::__construct( MODULE_CONTROL, 'LANGUAGE STRINGS' );
		...
		// File extension
  		...
  		if(!empty($_FILES['csv_file']['name']) && $extension == 'csv'){
		    ...
		    while(($csvData = fgetcsv($csvFile)) !== FALSE){
				...
				$count_row = $this->Language_string->count_all_by($conds_str);
			    
				// duplicate row in db
				if($count_row > 1){
					$this->Language_string->delete_by($conds_str);
				}
	 ...
	 */
	function edit_language_string( $id, $language_id ) {
