...
class App_settings extends BE_Controller {
	...
	function save( $id = false ) {
		...
		$item_config['item_location_township_id'] = $data['is_sub_location'];
		$this->Item_upload_config->save( $item_config, 1 );
		...
		// if is_subcat_subscription is checked,
		if ( $this->has_data( 'is_subcat_subscription' )) {
			$data['is_subcat_subscription'] = 1;
		} else {
			$data['is_subcat_subscription'] = 0;
		}
		...
		// if there is an error in inserting user data,	
			...
			if($data['is_paid_app'] == '1'){
				...
				$this->Module->save($is_show_menu,52);
			}
