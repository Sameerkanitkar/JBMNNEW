...
class Abouts extends BE_Controller {
	...
	function index( $id = "abt1" ) {
		...
		$this->data['action_title'] = get_msg('about_module');
	...
	function save( $id = false ) {
		...
		// faq pages
		if ( $this->has_data( 'faq_pages' )) {
			$data['faq_pages'] = $this->get_data( 'faq_pages' );
		}

		// terms and coditions
		if ( $this->has_data( 'terms_and_conditions' )) {
			$data['terms_and_conditions'] = $this->get_data( 'terms_and_conditions' );
		}
