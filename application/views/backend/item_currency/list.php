<div class="table-responsive animated fadeInRight">
	...
	<?php if ( !empty( $currencies ) && count( $currencies->result()) > 0 ): ?>
		...
		<?php foreach($currencies->result() as $currency): ?>
			...
			<tr>
				...
				<?php if ( $this->ps_auth->has_access( EDIT )): ?>
					...
					<td>
						<a href='<?php echo $module_site_url .'/edit/'. $currency->id; ?>'>
				...
				<?php if ( $this->ps_auth->has_access( PUBLISH )): ?>
					...
					<td>
						<?php if ( @$currency->status == 1): ?>
							<button class="btn btn-sm btn-success unpublish" id='<?php echo $currency->id;?>'>
