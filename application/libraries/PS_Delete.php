...
class PS_Delete {
	...
	function delete_user( $user_id ) {
		...
		$user_profile_photo = $this->CI->User->get_one($user_id)->user_profile_photo;
	...
	{		
		...
		if( $type_name == "model") {
			...
			//delete item -just update status - modified by PP @18Dec2020
			$itm_delete['status'] = -1;

			// if ( ! $this->CI->Item->delete( $type_id )) {
			if ( ! $this->CI->Item->save( $itm_delete,$type_id )) {
	...
	function delete_model_trigger( $model_id )
	{
		...
		// if the wallpaper list not empty
			...
			foreach ( $items as $item ) {
				...
				if ( !$this->CI->Item->save( $itm_delete,$item->id )) {
	...
	*/
	...
	function delete_location_township( $id )
	{		
		if ( ! $this->CI->Item_location_township->delete( $id )) {
		// if there is an error in deleting currency,
	 ...
	 * @param      <type>  $id     The identifier
	 */
	function delete_package( $package_id, $enable_trigger = false )
	{		
		if ( ! $this->CI->Package->delete( $package_id )) {
		// if there is an error in deleting package,
