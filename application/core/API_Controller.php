...
class API_Controller extends REST_Controller
{
	...
	*/
			...
			//if($user_obj->user_id != "") {
				...
				if($obj->id != "") {
					...
					if($this->Touch->save($data)) {
						...
						$this->success_response( get_msg( 'success_touch_count'), 201);
	...
	function custom_response_history( $data, $user_id, $require_convert = true )
	{
		...
		$app_object->is_subcat_subscription = $this->App_setting->get_one('app1')->is_subcat_subscription;
		...
		$item_config_object->item_type_id = $item_config->item_type_id;
		...
		$item_config_object->business_mode = $item_config->business_mode;
		$item_config_object->highlight_info = $item_config->highlight_info;
		$item_config_object->color_id = $item_config->color_id;
		$item_config_object->fuel_type_id = $item_config->fuel_type_id;
		$item_config_object->build_type_id = $item_config->build_type_id;
		$item_config_object->seller_type_id = $item_config->seller_type_id;
		$item_config_object->transmission_id = $item_config->transmission_id;
		$item_config_object->plate_number = $item_config->plate_number;
		$item_config_object->engine_power = $item_config->engine_power;
		$item_config_object->steering_position = $item_config->steering_position;
		$item_config_object->no_of_owner = $item_config->no_of_owner;
		$item_config_object->trim_name = $item_config->trim_name;
		$item_config_object->vehicle_id = $item_config->vehicle_id;
		$item_config_object->price_unit = $item_config->price_unit;
		$item_config_object->year = $item_config->year;
		$item_config_object->licence_status = $item_config->licence_status;
		$item_config_object->max_passengers = $item_config->max_passengers;
		$item_config_object->no_of_doors = $item_config->no_of_doors;
		$item_config_object->mileage = $item_config->mileage;
		$item_config_object->licence_expiration_date = $item_config->licence_expiration_date;
		$item_config_object->address = $item_config->address;

		$frontend_config_object = new stdClass;
		$frontend_config = $this->Frontend_config->get_one('fe1');
		$frontend_config_object->map_key = $frontend_config->map_key;
		$frontend_config_object->is_enable_video_setting = $frontend_config->is_enable_video_setting;
		$frontend_config_object->show_user_profile = $frontend_config->show_user_profile;
		$frontend_config_object->no_filter_with_location_on_map = $frontend_config->no_filter_with_location_on_map;
		$frontend_config_object->price_format = $frontend_config->price_format;
		$frontend_config_object->enable_notification = $frontend_config->enable_notification;
		$frontend_config_object->fcm_server_key = $frontend_config->fcm_server_key;
		$frontend_config_object->firebase_web_push_key_pair = $frontend_config->firebase_web_push_key_pair;
		$frontend_config_object->ad_client = $frontend_config->ad_client;
		$frontend_config_object->ad_slot = $frontend_config->ad_slot;
		$frontend_config_object->copy_right = $frontend_config->copy_right;
		$frontend_config_object->google_play_url = $frontend_config->google_play_url;
		$frontend_config_object->app_store_url = $frontend_config->app_store_url;
		$frontend_config_object->google_setting = $frontend_config->google_setting;
		$frontend_config_object->app_store_setting = $frontend_config->app_store_setting;
		$frontend_config_object->banner_src = $frontend_config->banner_src;
		$frontend_config_object->google_map = $frontend_config->google_map;
		$frontend_config_object->open_street_map = $frontend_config->open_street_map;
		$frontend_config_object->mile = $frontend_config->mile;
		$frontend_config_object->promote_first_choice_day = $frontend_config->promote_first_choice_day;
		$frontend_config_object->promote_second_choice_day = $frontend_config->promote_second_choice_day;
		$frontend_config_object->promote_third_choice_day = $frontend_config->promote_third_choice_day;
		$frontend_config_object->promote_fourth_choice_day = $frontend_config->promote_fourth_choice_day;
		$frontend_config_object->item_upload_ui1 = $frontend_config->item_upload_ui1;
		$frontend_config_object->item_upload_ui2 = $frontend_config->item_upload_ui2;
		
		$fe_default_language = array();
		$fe_exclude_language = array();
		$fe_exclude_lang = explode(',' ,trim($frontend_config->exclude_language));
		$fe_default_lang = trim($frontend_config->default_language);

		foreach($languages as $language){
			if(in_array($language['language_code'], $fe_exclude_lang)){
				$fe_exclude_language[] = array('language_code' => $language['language_code'], 'country_code' => $language['country_code'], 'name' => $language['name']);
			}

			if($language['language_code'] == $fe_default_lang){
				$fe_default_language = array('language_code' => $language['language_code'], 'country_code' => $language['country_code'], 'name' => $language['name']);
			}
		}

		$frontend_config_object->default_language = $fe_default_language;
		$frontend_config_object->exclude_language = $fe_exclude_language;
		...
		// for android type at in app purchase for item promote
		...
		// for ios type at in app purchase for item promote
		$conds_ios['type'] = "IOS";
		$conds_ios['status'] = "1";
		...
		$final_data->frontend_config_setting = $frontend_config_object;
