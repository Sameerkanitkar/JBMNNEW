  ...
  */
	...
	*/
	function count_item_by($conds = array()){
		...
		if ( isset( $conds['manufacturer_id'] )) {
			if ($conds['manufacturer_id'] != "" ) {
				if ($conds['manufacturer_id'] != '0') {
					$this->db->where( 'bs_items.manufacturer_id', $conds['manufacturer_id'] );	
				} 
		...
		if ( isset( $conds['model_id'] )) {
			if ($conds['model_id'] != "" ) {
				if ($conds['model_id'] != '0') {
					$this->db->where( 'bs_items.model_id', $conds['model_id'] );
				}
	...
	*/
	...
	function get_all_item_by_paid ( $conds = array(), $limit = false, $offset = false  ) {
		...
		if( isset($conds['max_year']) ) {
			if( $conds['max_year'] != "" ) {
				$this->db->where( 'bs_items.year <=', $conds['max_year'] );
			}	
		...
		if( isset($conds['min_year']) ) {
			...
			if( $conds['min_year'] != "" ) {
				$this->db->where( 'bs_items.year >=', $conds['min_year'] );
			}
		...
		if ( isset( $conds['fuel_type_id'] )) {
			...
			if ($conds['fuel_type_id'] != "") {
				if($conds['fuel_type_id'] != '0'){
					...
					$this->db->where( 'bs_items.fuel_type_id', $conds['fuel_type_id'] );	
				}
		...
		if ( isset( $conds['color_id'] )) {
			...
			if ($conds['color_id'] != "") {
				if($conds['color_id'] != '0'){
					...
					$this->db->where( 'bs_items.color_id', $conds['color_id'] );	
				}
		...
		if ( isset( $conds['build_type_id'] )) {
			...
			if ($conds['build_type_id'] != "") {
				if($conds['build_type_id'] != '0'){
					...
					$this->db->where( 'bs_items.build_type_id', $conds['build_type_id'] );	
				}
		...
		if ( isset( $conds['seller_type_id'] )) {
			...
			if ($conds['seller_type_id'] != "") {
				if($conds['seller_type_id'] != '0'){
					...
					$this->db->where( 'bs_items.seller_type_id', $conds['seller_type_id'] );	
				}
		...
		if ( isset( $conds['transmission_id'] )) {
			...
			if ($conds['transmission_id'] != "") {
				if($conds['transmission_id'] != '0'){
					...
					$this->db->where( 'bs_items.transmission_id', $conds['transmission_id'] );	
				}
		...
		if ( isset( $conds['is_discount'] )) {
			if ($conds['is_discount'] == "1") {
				$this->db->where( 'bs_items.discount_rate_by_percentage !=', '0' );				
				$this->db->where( 'bs_items.discount_rate_by_percentage !=', '' );			
			}	
		}
	...
	function get_all_item_by_normal_ads ( $conds, $limit = false, $offset = false  ) {
		...
		if(isset($conds['deal_option_id'])) {
   		...
   		if(isset($conds['model_id'])) {
			...
			if ($conds['model_id'] != "" || $conds['model_id'] != 0) {
		...
		$query2 = $this->db->get();
		// print_r(count($query2->result()));die;
		$query = array();
		$q1 = $query1->result();
		$q2 = $query2->result();
	...
	*/
	...
	function get_last_row(){
		...
		$sql = "SELECT * FROM " . $this->table_name . " ORDER BY `added_date` DESC LIMIT 1";
