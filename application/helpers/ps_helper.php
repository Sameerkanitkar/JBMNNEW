...
if ( ! function_exists( 'deep_linking_shorten_url' ))
{
	function deep_linking_shorten_url ($description,$title,$img,$id) {
		...
		$longUrl = $CI->Backend_config->get_one('be1')->dyn_link_deep_url.$item_name.'?'.'item_id='.$id.'&'.'item_name='.$item_name;
...
if ( ! function_exists( 'send_android_fcm' ))
{
	function send_android_fcm( $registatoin_ids, $data, $platform_names) 
    {
		...
		$condition_of_item_id = $CI->Item->get_one($id)->condition_of_item_id;
		$condition = $CI->Condition->get_one($condition_of_item_id)->name;
		...
		for ($i=0; $i <count((array)$platform_names) ; $i++) { 
    		...
    		if ($click_action =="" && $platform_names[$i] == "frontend" && $flag == "chat") {
    			...
    			$click_action = "";
    		} elseif ($click_action =="" && $platform_names[$i] == "frontend" && $flag == "verify_blue_mark") {
    			$click_action = "";
    	...
    	if ($flag == 'approval' || $flag == 'verify_blue_mark' || $flag == 'follow') {
    		// - Testing Start
			$noti_arr = array(
